...
def scan_stock(stock):
    ...
    print(f"{symbol} -> Swing Volume Ratio : {volume['ratio']}")
    print(f"{symbol} -> Yesterday Volume : {volume['volume']:,}")
    ...
    print(
        symbol,
        trend["breakout_age"],
        trend["trend_stage"].replace("🟢", "").replace("🟡", "").replace("🟠", "").replace("🔴", "")
    )
...
def get_swing_candidates():
    ...
    print(f"\nTop Swing Stocks Found : {len(rows)}")
